...
export function parseAttributeConfig(el) {
  ...
  try {
    ...
    warn('data-ghost payload is not valid JSON — discarded, using defaults');
    return null;
  }

  if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) {
    warn('data-ghost payload is not a JSON object — discarded, using defaults');
    return null;
  }

  for (const key of Object.keys(parsed)) {
    if (!KNOWN_COMPACT_KEYS.has(key)) {
      warn(`data-ghost payload has unknown key "${key}" — whole payload discarded`);
      return null;
    }
  }
  ...
  if ('m' in parsed) {
    if (typeof parsed.m !== 'string' || !['synthesize', 'freeze', 'off'].includes(parsed.m)) {
      warn('data-ghost "m" is not a valid mode — whole payload discarded');
      return null;
    }
  ...
  if ('d' in parsed) {
    if (typeof parsed.d !== 'number') { warn('data-ghost "d" is not a number — whole payload discarded'); return null; }
    config.delay = clamp(parsed.d, 0, 60000);
  }

  if ('h' in parsed) {
    if (typeof parsed.h !== 'number') { warn('data-ghost "h" is not a number — whole payload discarded'); return null; }
    config.hold = clamp(parsed.h, 0, 60000);
  }

  if ('r' in parsed) {
    if (typeof parsed.r !== 'number') { warn('data-ghost "r" is not a number — whole payload discarded'); return null; }
    config.rows = clamp(parsed.r, 0, 1000);
  }

  if ('p' in parsed) {
    if (typeof parsed.p !== 'boolean') { warn('data-ghost "p" is not a boolean — whole payload discarded'); return null; }
    config.poll = parsed.p;
  }

  if ('s' in parsed) {
    if (typeof parsed.s !== 'boolean') { warn('data-ghost "s" is not a boolean — whole payload discarded'); return null; }
    config.sync = parsed.s;
  }

  if ('l' in parsed) {
    if (typeof parsed.l !== 'boolean') { warn('data-ghost "l" is not a boolean — whole payload discarded'); return null; }
  ...
  if ('x' in parsed) {
    ...
    if (sanitized === undefined) { warn('data-ghost "x" is not an array — whole payload discarded'); return null; }
    config.except = sanitized;
  }

  if ('g' in parsed) {
    if (typeof parsed.g !== 'boolean') { warn('data-ghost "g" is not a boolean — whole payload discarded'); return null; }
    config.learning = parsed.g;
  }

  if ('n' in parsed) {
    if (typeof parsed.n !== 'string' || !COMPONENT_NAME_PATTERN.test(parsed.n)) { warn('data-ghost "n" is not a valid component name — whole payload discarded'); return null; }
    config.name = parsed.n;
  }

  if ('a' in parsed) {
    if (parsed.a === null || typeof parsed.a !== 'object' || Array.isArray(parsed.a)) {
      warn('data-ghost "a" is not a JSON object — whole payload discarded');
      return null;
    }
    const actionOverrides = {};
    for (const [action, fields] of Object.entries(parsed.a)) {
      if (!ACTION_NAME_PATTERN.test(action)) {
        warn(`data-ghost "a" has an invalid action name — whole payload discarded`);
        return null;
      }
      if (fields === null || typeof fields !== 'object' || Array.isArray(fields)) {
        warn(`data-ghost "a.${action}" is not a JSON object — whole payload discarded`);
        return null;
      }
      const override = {};
      for (const [key, value] of Object.entries(fields)) {
        if (!METHOD_OVERRIDE_KEYS.has(key)) {
          warn(`data-ghost "a.${action}" has unknown key "${key}" — whole payload discarded`);
          return null;
        }
        if (key === 'm') {
          if (typeof value !== 'string' || !['synthesize', 'freeze', 'off'].includes(value)) {
            warn(`data-ghost "a.${action}.m" is not a valid mode — whole payload discarded`);
            return null;
          }
          override.mode = value;
        } else if (key === 'd' || key === 'h' || key === 'r') {
          if (typeof value !== 'number') {
            warn(`data-ghost "a.${action}.${key}" is not a number — whole payload discarded`);
            return null;
          }
