...
export function createV4Bridge() {
  function subscribe(handlers) {
    return window.Livewire.interceptMessage(({ message, onSuccess, onError, onFailure, onCancel, onFinish }) => {
      // Blindagem: this callback runs inside Livewire's own MessageInterceptor
      // dispatch (confirmed live against Livewire 4.2.3 — see the isSkipped
      // case below), so anything it throws propagates out and kills every
      // Livewire request on the page, not just this message's skeleton. A
      // missing skeleton is an acceptable degradation; a dead Livewire app is
      // not. Neither this callback nor its registered onSuccess/onError/
      // onFailure/onCancel/onFinish handlers below ever call anything
      // equivalent to preventDefault() on Livewire's own dispatch, so
      // catching and returning here can't interrupt Livewire's own
      // sendRequest/succeed/fail flow for this message.
      try {
        // `Message.isSkipped()` only exists from Livewire ~4.3 onward, while this
        // package's constraint allows 4.0+, where the method is absent. Feature-
        // detect it instead of assuming the newest shape: a build without
        // `isSkipped` has no notion of a skipped message, so treating it as
        // "not skipped" is the correct reading there.
        if (typeof message.isSkipped === 'function' && message.isSkipped()) return;
        ...
        // Contain any throwing callback (blindagem), not just the isSkipped
        // case above — same failure class either way: an uncaught throw here
        // propagates out through Livewire's own dispatch and kills every
        // Livewire request on the page, not just this message's skeleton.
        if (isDebug()) console.warn('[ghostwire] v4 bridge interceptMessage callback threw — skeleton skipped for this message:', error);
      }
    });
  }
