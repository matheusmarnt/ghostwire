...
Ghostwire normally synthesizes a skeleton by walking the *current* DOM the instant a Livewire request starts — so a lazy-loaded component's very first paint, on a visitor's first-ever visit, has nothing to walk yet. Learning closes that gap: it remembers each component's Bone Tree locally, in the browser, and repaints it the next time that component is about to lazy-load, before any real content exists. This capture runs on every eligible commit the moment it starts, independent of the visible skeleton's own anti-flicker delay (`timing.delay`) — a commit that resolves instantly is captured exactly the same as one slow enough to actually show its own skeleton.
...
The flag only enables skeleton capture and produces no visible effect on its own — to see a learned skeleton painted, pair it with `#[Ghost(lazy: true)]` on your component (and Livewire's `#[Lazy]` if you want the skeleton to appear on the initial load).
...
The store is re-validated on every read. Anything that doesn't match the expected shape — a corrupted value, an entry that fails validation — is discarded silently, and the affected component simply learns again on its next visit. Storage is capped at a fixed 256 KB budget, with the least-recently-written entries evicted first once it's full.

## Schema versions

Learned trees are a cache, keyed to a schema version. A Ghostwire upgrade that bumps the version discards every previously learned tree — there's no migration path, because migrating a derived cache would be over-engineering: each entry is simply re-learned on the next visit, at the cost of one re-synthesis, and there's nothing a developer needs to do. The one part that doesn't take care of itself is the old value left in `localStorage`: it's removed the next time it's read, rather than orphaned there forever.
...
Pairing Livewire's own lazy loading with Ghost's — no `placeholder()` method, so Ghostwire's learned skeleton fills the gap once one exists for this component and breakpoint:
